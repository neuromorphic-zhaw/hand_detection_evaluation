...
if __name__ == '__main__':      
    ...
    compression = utils_lava.Compression.DENSE
    ...
    system = 'cpu_no_delta_input_cam' + str(cam_id)
    ...
    # Load model
    model_name = 'sdnn_1hot_smoothed_scaled_lowres_rmsprop_relu_no_delta_input'
    ...
    net = netx.hdf5.Network(net_config=act_model_path + 'model.net', skip_layers=0, input_shape=(344, 260, 1))
    ...
    encoder = utils_lava.Encoder(shape=net.inp.shape,
    ...
    sender.out_port.connect(encoder_output_extractor.in_port)
    # encoder.s_out.connect(encoder_output_extractor.in_port)

    run_condition = RunSteps(num_steps=num_steps, blocking=False)
        
    exception_proc_model_map = {utils_lava.Encoder: utils_lava.PyEncoderModelDense}
    ...
    for t in range(num_steps):
        ...
        quantized_input = quantize(input)
        # scale quandized_input to [-4096, 4096]

        scaled_input = np.interp(quantized_input, (quantized_input.min(), quantized_input.max()), (-8192, 8192))
        

        sender.send(scaled_input)        # This sends the input frame to the Lava network
    ...
    # plot sender output
    encoder_output_list[1].shape
    encoder_output_list[1]
    plt.imshow(np.swapaxes(encoder_output_list[53], 0, 1), cmap='gray')
    plt.colorbar()
