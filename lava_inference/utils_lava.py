...
class Encoder(AbstractProcess):
    ...
    Encoding looks at the input and sends it only when it is more than a positive threshold.

    Delta dynamics:
    s_out   = a_in if abs(a_in) > vth else 0  # spike mechanism
...
@requires(HostCPU)
class AbstractPyEncoderModel(PyLoihiProcessModel):
    ...
    def encode(self, act_input):
        s_out = np.where(np.abs(act_input) >= self.vth, act_input, 0)
        if self.a_max > 0:
...
@tag('dense_out')
class PyEncoderModelDense(AbstractPyEncoderModel):
    ...
    def run_spk(self):
        ...
        a_in_data = np.left_shift(self.a_in.recv().astype(int),
                                  self.spike_exp)
        self.s_out_buf = self.encode(a_in_data)


@implements(proc=Encoder, protocol=LoihiProtocol)
@tag('sparse_out')
class PyEncoderModelSparse(AbstractPyEncoderModel):
